Feature: HMRC-KBV-POST-Session-HappyPath.feture
    ...
    Scenario Outline: Happy Path - Post Request to Session Endpoint
        Given I send a Valid POST request to the session endpoint with <contentType> and <accept> and <x-forwarded-for>
        Then I should receive a response with <statusCode>
        Examples:
            | statusCode | contentType      | accept           | x-forwarded-for |
