...
describe("HMRC KBV Happy Path", () => {
    ...
    it.each([
        ...
        "AA000005A",
        "AA000006A",
    ])("Successful Request Test for $selectedNino - VC Generation for User with 3 HMRC KBV Questions", async (selectedNino: string) => {
        ...
        await postRequestToAnswerEndpoint(getQuestionResponse.data.questionKey, sessionId);
        // Answer Question 2
        const getSecondQuestionResponse = await getRequestToQuestionEndpoint(sessionId, 200);
        await postRequestToAnswerEndpoint(getSecondQuestionResponse.data.questionKey, sessionId);
        // Answer Question 3
        const getThirdQuestionResponse = await getRequestToQuestionEndpoint(sessionId, 200);
        await postRequestToAnswerEndpoint(getThirdQuestionResponse.data.questionKey, sessionId);
        // Validate no more questions requested
        await getRequestToQuestionEndpoint(sessionId, 204);
        // Get Authorization Code
        const authResponse = await getRequestAuthorisationCode(state, clientId, sessionId);
        // Get Token Private Key JWT
        const tokenResponse = await getTokenRequestPrivateKeyJWT(authResponse.data.authorizationCode.value, sessionId);
        // Get Access Token
        ...
        const verifiableCredentialResponse = await postRequestHmrcKbvCriVc(accessTokenResp.data.access_token);
        await validateJwtToken(verifiableCredentialResponse.data, getClaimsResponse, 2);
    });
    ...
    it.each([
        ...
        const verifiableCredentialResponse = await postRequestHmrcKbvCriVc(accessTokenResp.data.access_token);
        await validateJwtToken(verifiableCredentialResponse.data, getClaimsResponse, 0);
    });
